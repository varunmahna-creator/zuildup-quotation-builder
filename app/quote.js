...
async function bootForm() {
  ...
  // ---- Feedback (deploy stage) ----
  // Sends a comment + redacted state summary to /feedback (server forwards to Discord webhook).
  // Redaction: NO customer name/phone/address — just shape (build mode, row counts, draft flag).
  function buildStateSummary(s) {
    const o = s || {};
    const safeRows = Array.isArray(o.rows) ? o.rows.length : 0;
    const unedited = Array.isArray(o.rows)
      ? o.rows.filter(r => !r || !r.override || (!r.override.rate && !r.override.label)).length
      : 0;
    return {
      build_mode: o.build && o.build.mode,
      plot_l: o.build && o.build.plotL,
      plot_b: o.build && o.build.plotB,
      coverage: o.build && o.build.cov,
      floors: o.build && o.build.floors,
      lift: !!(o.build && o.build.liftEnabled),
      basement: !!(o.build && o.build.hasBasement),
      cost_per_sqft: o.pricing && o.pricing.costPerSqft,
      structure_rate: o.pricing && o.pricing.structureRate,
      scope: o.scope,
      draft: !!o.draft,
      row_count: safeRows,
      unedited_rows: unedited,
      quote_id: o.quoteId,
      created_at: o.createdAt,
      active_slot: (function(){ try { return QuoteStorage.activeId(); } catch(_) { return null; } })(),
    };
  }
  const fbBtn    = document.getElementById('feedback-btn');
  const fbModal  = document.getElementById('feedback-modal');
  const fbCancel = document.getElementById('feedback-cancel');
  const fbSubmit = document.getElementById('feedback-submit');
  const fbText   = document.getElementById('feedback-text');
  function fbClose() { if (fbModal) fbModal.classList.remove('open'); }
  if (fbBtn && fbModal) {
    fbBtn.onclick = () => {
  ...
  if (fbCancel) fbCancel.onclick = fbClose;
  if (fbModal) fbModal.addEventListener('click', (e) => { if (e.target === fbModal) fbClose(); });
  if (fbSubmit) fbSubmit.onclick = async () => {
    const comment = (fbText && fbText.value || '').trim();
    if (!comment) { toast('Please write a comment first', 'err'); return; }
    ...
    try {
      ...
      if (resp.status === 503) {
        toast('Feedback channel not yet wired — try again later', 'err');
      } else if (!resp.ok) {
        toast('Feedback failed (HTTP ' + resp.status + ')', 'err');
      } else {
        toast('Thanks!');
        fbClose();
      }
    } catch (e) {
      toast('Feedback failed: ' + e.message, 'err');
    } finally {
      fbSubmit.disabled = false;
      fbSubmit.textContent = 'Send';
    }
  };
